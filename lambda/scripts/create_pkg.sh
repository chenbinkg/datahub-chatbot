...
if [ -f "$parent_dir/infra/2-chatbot-interface/lambda_$function_name.zip" ]; then
  rm "$parent_dir/infra/2-chatbot-interface/lambda_$function_name.zip"
fi
...
# zip -r9 "$parent_dir/infra/2-chatbot-interface/$function_name.zip" $path_cwd/env_$function_name/lib/*/site-packages/.
...
if [ -f "$FILE" ]; then
  ...
  if [ -d "$site_pkg_dir" ]; then
    # zip -r9 "$parent_dir/infra/2-chatbot-interface/$function_name.zip" "$site_pkg_dir"
    echo "Zipping contents of: $site_pkg_dir"
    cd $site_pkg_dir
    zip -r9 "$parent_dir/infra/2-chatbot-interface/lambda_${function_name}.zip" .
    cd $path_cwd
  else
...
# zip -g "$parent_dir/infra/2-chatbot-interface/$function_name.zip" "$path_cwd/lambda_functions/$function_name/$function_name.py"
target_dir="$path_cwd/lambda_functions/$function_name"
if [ -d "$target_dir" ]; then
  # zip -r9 "$parent_dir/infra/2-chatbot-interface/lambda_${function_name}.zip" "$target_dir"
